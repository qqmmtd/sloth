...
StandardSodukuSolution::StandardSodukuSolution()
{
}

/* bit count */
int StandardSodukuSolution::bitcount(int v) {
    int count;
    for (count = 0; v; ++count) {
        v &= v - 1;
    }
...
int StandardSodukuSolution::bit2num(int v) {
    int i;
    for (i = 0; v; ++i) {
        v >>= 1;
    }
...
int StandardSodukuSolution::countPossibleValues(Unit grid[], int idx) {
...
int StandardSodukuSolution::toReadableValue(Unit grid[], int idx) {
    if (grid[idx].values < 0) {
        return -(grid[idx].values);
    }
...
void StandardSodukuSolution::dumpGrid(Unit grid[]) {
    int i, j;
 ...
 */
int StandardSodukuSolution::reducePossibleValues(Unit grid[]) {
    if (!grid) {
        return -1;
    }
...
int StandardSodukuSolution::recursiveTryPossibleValues(Unit grid[]) {
    if (!grid) {
        return -1;
    }
    ...
    for (i = 0; i < C_ROW; ++i) {
        if (grid[mini].values & (1 << i)) {
            ...
            if (recursiveTryPossibleValues(ngrid) == 0) {
                ...
                memcpy(grid, ngrid, sizeof(Unit) * C_UNIT);
                return 0;
            }
        }
    }
