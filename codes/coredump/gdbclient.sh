...
if [ -z "$ANDROID_BUILD_TOP" ]; then
  >&2 echo '$ANDROID_BUILD_TOP is not set. Source build/envsetup.sh and choosecombo.'
  exit 1
fi
...
# exits 0 when process is not traced
...
function gdbclient() {
  ...
  if [ -z "$PID" ]; then
    ...
    echo "Usage: gdbclient -c <exename> <coredump>"
    exit -1
  fi
#  local DEVICE=$(adb_get_product_device)
  local DEVICE=${ANDROID_PRODUCT_OUT##*/}
...
if [[ ! $PID == "-c" ]]; then
  ...
  gdbwrapper $GDB "$OUT_ROOT/gdbclient.cmds" "$LOCAL_EXE_PATH" "$3"
}
